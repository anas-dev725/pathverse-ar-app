...
import { saveUserProfile, getUserProfile, findProfileByEmail } from '../database/database';
...
export default function SplashScreen({ onFinish }) {
  ...
  useEffect(() => {
    ...
    // 5. 3D loops for slider visualizations (Floating & Rotating)
    Animated.loop(
      Animated.sequence([
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(rotateYAnim, { toValue: 1, duration: 3200, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.timing(rotateYAnim, { toValue: -1, duration: 3200, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
      ])
    ).start();
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(rotateXAnim, { toValue: 1, duration: 3800, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
        Animated.timing(rotateXAnim, { toValue: -1, duration: 3800, easing: Easing.inOut(Easing.ease), useNativeDriver: true }),
      ])
    ).start();
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(scanLineAnim, { toValue: 1, duration: 2200, easing: Easing.linear, useNativeDriver: true }),
        Animated.timing(scanLineAnim, { toValue: 0, duration: 0, useNativeDriver: true }),
      ])
    ).start();

    Animated.loop(
      Animated.timing(rotateZAnim, { toValue: 1, duration: 15000, easing: Easing.linear, useNativeDriver: true })
    ).start();
